...
function writeFileSafely(path, content) {
  ...
  try { renameSync(tempPath, path); } catch { rmSync(path, { force: true }); renameSync(tempPath, path); }
}

const registryEntries = Array.from(
  registrySource.matchAll(/path:\s*"([^"]+)"[\s\S]*?lastUpdated:\s*"([^"]+)"[\s\S]*?status:\s*"([^"]+)"/g),
).map((match) => ({
  path: match[1], lastmod: toIsoDate(match[2]), status: match[3],
...
const blogEntries = Array.from(blogsSource.matchAll(/slug:\s*"([^"]+)"[\s\S]*?date:\s*"([^"]+)"/g)).map((match) => ({
  path: `/insights/${match[1]}`, lastmod: toIsoDate(match[2]), priority: "0.7", changefreq: "monthly",
}));

const serviceEntries = Array.from(servicesSource.matchAll(/slug:\s*"([^"]+)"/g)).map((match) => {
  const slug = match[1];
  return { path: slug.endsWith("-gurgaon") ? `/${slug}` : `/${slug}-gurgaon`, lastmod: new Date().toISOString().slice(0, 10), priority: "0.9", changefreq: "monthly" };
});

const franchiseCityEntries = Array.from(franchiseCitiesSource.matchAll(/city\("([^"]+)"\s*,\s*"([^"]+)"/g)).map((match) => ({
  path: `/franchise/${slugify(match[1])}`, lastmod: new Date().toISOString().slice(0, 10), priority: "0.8", changefreq: "monthly",
}));
...
const gurgaonLocalEntry = { path: "/gurgaon-salon-near-me", lastmod: new Date().toISOString().slice(0, 10), priority: "0.9", changefreq: "weekly" };
const waxingEntry = { path: "/waxing-gurgaon", lastmod: new Date().toISOString().slice(0, 10), priority: "0.9", changefreq: "weekly" };
const mundanEntry = { path: "/mundan-ceremony-gurgaon", lastmod: new Date().toISOString().slice(0, 10), priority: "0.9", changefreq: "weekly" };

const routes = uniqueByPath([
  ...registryEntries, ...serviceEntries, gurgaonLocalEntry, waxingEntry, mundanEntry,
  franchiseDirectoryEntry, franchiseMediaKitEntry, ...franchiseCityEntries, ...blogEntries,
]);
...
const localPages = uniqueByPath([
  ...registryEntries, ...serviceEntries, gurgaonLocalEntry, waxingEntry, mundanEntry,
  franchiseDirectoryEntry, franchiseMediaKitEntry, ...franchiseCityEntries,
]).filter((entry) => !entry.path.startsWith("/insights/"))
  .map((entry) => `- [${entry.path === "/" ? "Home" : entry.path.slice(1).replaceAll("-", " ")}](${entry.path})`).join("\n");

const latestBlogs = blogEntries.sort((a, b) => b.lastmod.localeCompare(a.lastmod)).slice(0, 20)
  .map((entry) => `- [${entry.path.replace("/insights/", "").replaceAll("-", " ")}](${entry.path})`).join("\n");

const llms = `# KidSalonia

> KidSalonia is Gurugram's premium kids and family salon for gentle haircuts, mundan, nail art, manicure, pedicure, skin care, party grooming, and child-friendly salon experiences. KidSalonia also offers at-home waxing in Gurgaon subject to service-area and appointment availability, and Mundan ceremony both at home in supported Gurgaon locations and at the Sector 67 salon.
...
- Mundan at home or salon Gurgaon: ${site}/mundan-ceremony-gurgaon

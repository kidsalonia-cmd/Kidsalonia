...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const siteUrl = 'sc-domain:kidsalonia.com';
    const accessToken = await getAccessToken(serviceAccount, ['https://www.googleapis.com/auth/webmasters.readonly']);

    if (mode === 'pages') {
      // Page-level aggregation — total clicks & impressions per page (all time available in GSC, last 16 months)
      const endDate = new Date();
      const startDate = new Date();
      startDate.setMonth(endDate.getMonth() - 16);
      ...
      for (const row of (data.rows || []) as GSCRow[]) {
        const page = row.keys[0]
          .replace('https://www.kidsalonia.com', '')
          .replace('https://kidsalonia.com', '') || '/';
    ...
    for (const row of (currentData.rows || []) as GSCRow[]) {
      ...
      if (!keywordMap[keyword] || row.position < keywordMap[keyword].position) {
        keywordMap[keyword] = {
          ...
          page: page.replace('https://www.kidsalonia.com', '').replace('https://kidsalonia.com', '') || '/',
        };
      }
    }
